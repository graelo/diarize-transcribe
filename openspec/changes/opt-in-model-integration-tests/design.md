...
Add a dedicated GitHub Actions workflow using ordinary `pull_request` events, not `pull_request_target`, with only read access and no secrets. Trigger on PR open, update, reopen, label, and unlabel events; run the model test only when `run-model-tests` is present. A stable job/check name is required in the repository branch ruleset. GitHub reports a skipped job as successful, so the check passes without running when the label is absent; including the `unlabeled` event re-evaluates the check after the opt-in is withdrawn. Use PR-scoped concurrency so an updated or unlabeled PR can cancel a superseded model run.
