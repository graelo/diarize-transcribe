...
The package SHALL provide a command-line application installed as the `diarize-transcribe` executable, with `--help` and `--version` options, required audio-input and transcript-output file parameters, and a `--chunk-seconds` option that defaults to 300 seconds and accepts only finite positive values. The former `diarize` executable SHALL NOT be installed.
...
- **WHEN** a user supplies a zero, negative, or non-finite value for `--chunk-seconds`
- **THEN** the command reports a parameter error and does not begin model inference
...
- **WHEN** a user supplies a finite positive value for `--chunk-seconds`
